<template>
  <p>
    <i>
      ...
      proposal on how to divide the coins, risking the same fate as his
  ...
  <p>
    ...
    that $P_k$ is the current captain, i.e., that all pirates $P_i$ with $1 \le
    i \lt k$ have been thrown overboard. In what follows, we will denote the
    proposal made by $P_k$ as $\Vecg{\mu}^\SupPar{k} = (\mu^\SupPar{k}_k,
  ...
  <p>
    As the first step to solve the problem using backward induction, we assume
    that all proposals are rejected until only two pirates remain: $P_{N - 1}$
    and $P_N$. Since $P_{N - 1}$ is allowed to vote on any proposal he makes, he
    has full control over the coin division since his vote comprises half of the
    total number of votes. Being selfish, $P_{N - 1}$ takes all the coins for
    himself, leaving $P_N$ with none &mdash; an outcome that $P_N$ has no choice
    but to accept. Therefore:
  </p>
  ...
  <p>
    ...
    - 1}$ will vote for any proposal that offers him at least one coin. This
  ...
  <p>
    ...
    0$), they will vote for any proposal that offers them at least one coin, so
  ...
  <p>
    Equation \eqref{C-1-1} is a counterintuitive result, especially when $C$ is
    much larger than $N$. As an example, taking $C = 1000$ and $N = 10$, we
    obtain $\mu^\SupPar{1}_1 = 1000 - \Floor{9 / 2} = 996$, i.e., $P_1$ can keep
    $996$ coins for himself!
  </p>

  <p>
    Above, we implicitly assumed that the number of coins $C$ was sufficiently
    large to allow $P_1$ to make a proposal that would be accepted, i.e., that
    $C \ge \Floor{(N - 1) / 2}$ and therefore $\mu^\SupPar{1}_1 \ge 0$. In the
    special case where $C = \Floor{(N - 1) / 2}$, we have $\mu^\SupPar{1}_1 =
    0$, meaning $P_1$ has no choice but to give all the coins to the other
    pirates in order to save his life. When $C \lt \Floor{(N - 1) / 2}$, $P_1$
    will find himself in trouble because at least one pirate $P_i$ whose vote he
    needs to pass his proposal will end up without any coins, even if $P_1$
    proposes giving all the coins away. From $P_i$'s perspective, voting to
    throw $P_1$ overboard will either lead to a division of the coins that
    leaves him with zero coins or to a division that leaves him with at least
    one coin. In the first scenario, $P_i$ will vote to throw $P_1$ overboard
    out of spite, and in the second scenario, he will vote to throw $P_1$
    overboard to get at least one coin. In both cases, $P_1$'s proposal will
    fail, and the next most senior pirate will become the new captain. This will
    continue happening until we reach the case where a captain $P_k$ is able to
    give all the coins to the other pirates while keeping none for himself in
    order to save his life, i.e., when $C = \Floor{(N - k) / 2}$.
  </p>
</template>
